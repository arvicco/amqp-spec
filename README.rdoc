...
In order to run setup/teardown hooks inside the EM loop, you'll need to use em_before{}
and em_after{} hooks that run inside the EM loop but before/after your example block.
You don't need to call 'done' inside em_before/em_after, otherwise it'll shut down reactor.
