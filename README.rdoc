...
Also please note interface change compared to version 0.2.7, you should now use standard form
of setters and getters: .default_options, .default_options=, .default_timeout and
.default_timeout instead of combined (macros) form. Macros form is still supported for now,
in order not to break existing specs, but it is deprecated and will be removed in future
versions of AMQP-Spec.
...
before{} and after{} hooks in your example groups including AMQP::Spec. Each of these hooks
...
tear down using these hooks will be lost as each example will run in a separate EM loop.
In order to run setup/teardown hooks inside the EM loop, you'll need to use before_amqp{}
and after_amqp{} hooks that run inside the EM loop but before/after AMQP loop proper
(TODO: these hooks are currently not implemented!)
...
examples inside em block instead of amqp. before{} and after{} hooks should be finished
