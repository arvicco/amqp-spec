...
the example, they override the defaults. Only one set of default options per example group is used, different
example groups have different defaults (this is different compared to EM-Spec, where default timeout is global).
...
Finally, you can include AMQP::EMSpec in your describe block. This will run all the examples in group
